...
namespace IvansCompany.Handlers
{
	public partial class Site : SiteBase
    {
    }
    
    public class SiteBase : IvansCompany.Web.PageBase
    {
        ...
        public static string Copyright
        {
            get
            {
                return "&copy; 2018 IvansCompany. ^Copyright^All rights reserved.^Copyright^";
            }
        }
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            if (!(String.IsNullOrEmpty(link)))
            {
                ...
                string[] permalink = enc.Decrypt(link.Replace(" ", "+").Split(',')[0]).Split('?');
            ...
            if (contentInfo.TryGetValue("PageContent", out s) && (_pageContent != null))
            {
                if (_isTouchUI)
                {
                    string prefetch = PreparePrefetch(s);
        ...
        protected virtual void InitializeSiteMaster()
        {
            ...
            string themePath = Server.MapPath("~/App_Themes/IvansCompany");
            if (Directory.Exists(themePath))
            	foreach (string stylesheetFileName in Directory.GetFiles(themePath, "*.css"))
                {
                    ...
                    if (!(fileName.Equals("_Theme_Aquarium.css")))
                    {
                        ...
                        link.Href = ("~/App_Themes/IvansCompany/" + fileName);
