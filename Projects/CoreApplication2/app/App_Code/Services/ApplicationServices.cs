...
namespace IvansCompany.Services
{
    ...
    public class GetIdentityServiceRequestHandler : ServiceRequestHandler
    {
        ...
        public override object HandleRequest(DataControllerService service, JObject args)
        {
            ...
            if ((user == null) || !((HttpContext.Current.Request.QueryString["force"] == "false")))
            {
                ...
                res.Redirect(String.Format("{0}?ReturnUrl={1}&_accMan=login", FormsAuthentication.LoginUrl, HttpUtility.UrlEncode((HttpContext.Current.Request.Path + "?force=false"))), true);
            }
            else
            {
                UserTicket ticket = ApplicationServices.Current.CreateTicket(user, null);
                ...
                res.Clear();
                res.Write("<html><body><script>function add() { window.CloudOnTime.invoke(\'addidentity\', \'");
                res.Write(HttpUtility.UrlPathEncode(JsonConvert.SerializeObject(ticket, Formatting.None)));
                res.Write("\');} if (typeof(CefSharp) == \'object\') CefSharp.BindObjectAsync(\'CloudOnTime\').th" +
                        "en(add); else add();</script></body></html>");
                res.End();
            }
            return null;
        }
    ...
    public class GetManifestServiceRequestHandler : ServiceRequestHandler
    {
        ...
        public override object HandleRequest(DataControllerService service, JObject args)
        {
            ...
            JObject settings = services.DefaultSettings;
            ...
            if (HttpContext.Current.Request.IsAuthenticated)
            {
                ...
                // add config.js
                string pageContent = GetConfigJS();
                response["config"] = JObject.FromObject(ManifestFile.GetConfig(pageContent));
                // build required js
                bool combinedScript = AquariumExtenderBase.EnableCombinedScript;
                AquariumExtenderBase.EnableCombinedScript = true;
                foreach (ScriptReference s in AquariumExtenderBase.StandardScripts(true))
                	if (!(String.IsNullOrEmpty(s.Path)))
                    	reqFiles.Add(ManifestFile.FromPath(s.Path));
                    else
                    {
                        ManifestFile f = ManifestFile.FromResource(s.Name);
                        files.Add(f);
                        reqFiles.Add(f);
                    }
                AquariumExtenderBase.EnableCombinedScript = combinedScript;
                JObject require = new JObject();
                response["require"] = require;
                require["js"] = JArray.FromObject(reqFiles);
                reqFiles.Clear();
                // build required css
                foreach (string css in services.EnumerateTouchUIStylesheets())
        ...
        void AddSiteFiles(List<ManifestFile> files, string siteFolder)
        {
            ...
            foreach (string file in Directory.EnumerateFiles(Path.Combine(rootPath, siteFolder), "*.*", SearchOption.AllDirectories))
            {
                Match m = Regex.Match(file, "\\.(?\'Culture\'\\w\\w(-\\w\\w)?)\\.(\\w+)$");
        ...
        {
            Match m = Regex.Match(GetHomePageContent(), "<\\/footer><script >\\s*(?\'Config\'[\\S\\s]+?)<\\/script>");
            if (!(m.Success))
        ...
        public static string GetHomePageContent()
        {
            ...
            string homePage = ApplicationServices.ResolveClientUrl(ApplicationServices.HomePageUrl);
            HttpWebRequest request = WebRequest.CreateHttp(homePage);
    ...
    public class GetManifestFileServiceRequestHandler : ServiceRequestHandler
    {
        ...
        public override object HandleRequest(DataControllerService service, JObject args)
        {
            ...
            string requestedFile = ((string)(args["file"]));
            ...
            if (requestedFile == "js\\host\\config.js")
                ...
                {
                    string resourceName = requestedFile.Substring(11);
                	...
                	if (requestedFile == "js\\daf\\add.min.js")
                    	HttpContext.Current.Response.Write(ApplicationServices.Current.AddScripts());
                    else
                    	if (requestedFile == "css\\daf\\add.min.css")
                        	HttpContext.Current.Response.Write(ApplicationServices.Current.AddStyleSheets());
                        else
                        {
                            string p = context.Server.MapPath(("~\\" + requestedFile));
                            if (File.Exists(p))
                            	using (FileStream f = File.OpenRead(p))
                                	f.CopyTo(HttpContext.Current.Response.OutputStream);
                            else
    ...
    public class ApplicationServicesBase
    {
        ...
        {
            IvansCompany.Services.ApplicationServices.Initialize();
        }
        ...
        public static void RegisterStandardMembershipAccounts()
        {
            ...
            if (Membership.GetUser("admin") == null)
            {
                ...
                admin = Membership.CreateUser("admin", "admin123%", "admin@IvansCompany.com", "ASP.NET", "Code OnTime", true, out status);
                user = Membership.CreateUser("user", "user123%", "user@IvansCompany.com", "ASP.NET", "Code OnTime", true, out status);
